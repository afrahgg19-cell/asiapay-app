...
if not df.empty:
  ...
  # 🌟 ميزة تعديل أو حذف أي صف محدد من السجل
  with st.expander("✏️ تعديل أو حذف عملية سابقة من السجل"):
    ...
    if selected_row_idx is not None:
      ...
      with st.form("edit_row_form"):
        ...
        new_edit_type = st.selectbox("نوع العملية", ["إيداع للمحفظة", "سحب كاش", "استرجاع للمحفظة"], index=["إيداع للمحفظة", "سحب كاش", "استرجاع للمحفظة"].index(current_row['نوع العملية']) if current_row['نوع العملية'] in ["إيداع للمحفظة", "سحب كاش", "استرجاع للمحفظة"] else 0)
        ...
        if submit_edit:
          ...
          # إعادة حساب أرصدة المحفظة بالترتيب لضمان دقة الأرقام
          running_bal = 0.0
...
if "حالة الديون" in df.columns:
  ...
  if not debts_df.empty:
    ...
    if st.button("✅ تم التسديد (تحديث وإزالة من المديونية)"):
      real_idx = int(selected_debt.split("رقم السجل (")[1].split(")")[0])
