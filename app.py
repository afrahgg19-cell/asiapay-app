  ...
  if uploaded_file_8 is not None and uploaded_file_9 is not None:
    try:
      ...
      name_col = (
          ...
          else combined_df.columns[min(1, len(combined_df.columns) - 1)]
      )
...
with tab3:
  ...
  if (
    ...
    name_col = (
        ...
        else df_combined.columns[min(1, len(df_combined.columns) - 1)]
    )
...
# ====================================================
# التبويب الرابع: KPI (قراءة دقيقة لـ Business to Business وتحويل النصوص من T لأرقام)
# ====================================================
with tab_kpi:
  st.markdown("### 📈 لوحة مؤشرات الأداء (KPI)")
  st.write(
      "تجميع Short Code (عمود H)، الاسم بالعربي (عمود F)، عد العمليات"
      " من عمود C، واستخراج وتحويل مبالغ business to business transfer من عمود T النصي إلى أرقام صافية."
  )
